...
Commander, every supply I deliver could mean the difference between your survival and becoming another casualty of the Prometheus Incident. I await your orders.`;
  ...
  private async handleMove(direction: string) {
    ...
    if (newRoomData) {
      ...
      if ((newRoomData as any).requires_maintenance_keycard) {
      ...
      if ((newRoomData as any).requires_maintenance_keys) {
        const items = await this.db.getItemsInLocation('inventory');
      ...
      if ((newRoomData as any).locked && !(newRoomData as any).requires_maintenance_keycard && !(newRoomData as any).requires_maintenance_keys) {
        return {
  ...
  private async handleEquipFromBunker(itemName?: string) {
    const gameState = await this.db.getGameState();
    if (!gameState) throw new Error('Game not initialized');
    ...
    // Find specific item
    const targetItem = bunkerInventory.find(item => 
  ...
  private async initializeWorld() {
    ...
    for (const item of BASEMENT_ITEMS) {
      ...
      `, [
        ...
        item.energyCost, 
        item.location,
        (item as any).foodValue || 0,
        (item as any).energyValue || 0,
        (item as any).waterValue || 0
      ]);
    }
